...
{
    /**
     * Find subsequences that match the last (ngram_size - 1) generated tokens.
     *The next-tokens of those matching
     * subsequences should be banned from the next-token logits, such that
     *existing N-grams in current generated
     * sequence will not be generated again.
     *
     * Note 1: no-repeat restriction is per-beam instead of across-beam.
     * Note 2: since beam search results are stored and retrieved by backtracking
     *(parent_ids), the entire sequence for
     * the current can only be obtained by backtracking all the way back.
     * Note 3: for greedy search, actually a more efficient implementation can be
     *adopted (since we're not constrained
     * by the beam backtrack retrieval, we can have all threads loading into
     *shared mem in parallel AND use normal order
     * traversal). But for simplicity and consistency, greedy search and beam
     *search implementation are kept the same.
     *
     * The overlap between adjacent threads indicates wasted global memory access.
     *Used shared memory instead.
     * Shared memory benefit is more significant as ngram_size increases. Shared
     *memory reuse is for
     * in-bound positions only. For leftside out-of-boundary tokens, access by
     *global memory.
     */
    ...
    // shared mem layout: per-thread token that each thread is mapped to, plus
    // (ngram_size - 1) extra tokens beyond
    // block boundary, plus (ngram_size - 1) most recent generated tokens in the
    // beam
    ...
    // retrieve the entire beam by backtracking from last token to current token
    // (in reverse order)
    // single thread vs parallel thread is equivalent as it's bound by the longest
    // iteration
    if (threadIdx.x == 0)
    {
        ...
        // write to shared mem in reverse order; boundary condition when thread
        // block covers more than step
        ...
        for (auto curr_idx = step - 1; curr_idx >= output_idx; curr_idx--)
        {
            ...
            // before reaching the part of current block, traverse only; after that,
            // record the tokens
    ...
    if (ban_ngram)
    {
        auto const banned_token = shared_tokens[threadIdx.x + no_repeat_ngram_size - 1]; // ban the last
                                                                                         // token in the
                                                                                         // ngram
        logits[local_batch_idx * beam_width * vocab_size_padded + beam_idx * vocab_size_padded + banned_token]
            = static_cast<T>(-INFINITY); // note: "logits" passed in is already with
                                         // batchxbeam offset
    }
}
...
{
    // each input in the local batch can have different no_repeat_ngram_size. Use
    // max for shmem allocation
    // getting the max of current batch and allocate shmem as needed is ideal. But
    // here the ngram_buf is on GPU, while
    // this max value is on CPU for kernel launch. Instead of really finding the
    // max and extra CPU-GPU memcpy, we simply
    ...
    // dynamically allocate shared memory of int[blockDim + 2*(ngram_size - 1)],
    // where ngram_size - 1 is for boundary
