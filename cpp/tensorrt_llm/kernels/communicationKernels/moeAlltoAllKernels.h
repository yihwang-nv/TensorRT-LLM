...
{
    void const* src_data;   // Source data pointer [local_num_tokens,
                            // elements_per_token]
    int element_size;       // Size of each element in bytes
    int elements_per_token; // Number of elements per token (e.g., hidden_size,
                            // top_k)
};
...
struct DispatchKernelPointers
{
    ...
    void* recv_buffers[kMaxRanks][kMaxPayloads]; // 2D array of receive buffer
                                                 // pointers
    int payload_bytes_per_token[kMaxPayloads];   // Bytes per token for each payload

    // Completion flags for synchronization
    uint32_t* completion_flags[kMaxRanks]; // If completion_flags[target_rank][source_rank] ==
                                           // *flag_val, then source
                                           // rank has signaled the target rank
    uint32_t* flag_val;                    // The value of the flag for this round (stored on the
                                           // local rank)

    // Local aux data pointers
    int* send_counters;            // [ep_size] How many tokens have been sent to each target
                                   // rank
    int* recv_counters[kMaxRanks]; // How many tokens have been received from each
                                   // source rank. Each rank has [ep_size]
                                   // counters
    int* local_token_counter;      // Atomic counter for completed tokens

    // Top-K compact routing info per local token (size: [local_num_tokens,
    // top_k])
    int* topk_target_ranks; // target rank per k, -1 for duplicates
    int* topk_send_indices; // dst index per k, -1 for duplicates
};

// Combine kernel pointers - non-const output in src_data_ptrs[0], const recv
// buffers
struct CombineKernelPointers
{
    // Payload pointers
    void* src_data_ptrs[kMaxPayloads];                 // src_data_ptrs[0] is output
    void const* recv_buffers[kMaxRanks][kMaxPayloads]; // 2D array of receive
                                                       // buffer pointers (const)
...
{
    bool one_block_per_token; // True: one block per token, False: one warp per
                              // token
    ...
    int num_experts_per_rank; // Number of experts per rank (num_experts /
                              // ep_size)

    // Token configuration
    int local_num_tokens;    // Number of tokens on this rank
    int max_tokens_per_rank; // Maximum tokens per rank for pre-allocation TODO:
                             // Rename to runtime_max_tokens_per_rank
    ...
    int* send_counters;       // [ep_size] atomic counters - tracks tokens sent to each
                              // target rank
    int* topk_target_ranks;   // Top-K compact routing info per local token (size:
                              // [local_num_tokens, top_k]), target rank
                              // per k, -1 for duplicates
    int* topk_send_indices;   // Top-K compact routing info per local token (size:
                              // [local_num_tokens, top_k]), dst index
                              // per k, -1 for duplicates

    // Distributed aux data and recv buffers
    int* recv_counters[kMaxRanks];               // tracks tokens received from each source
                                                 // rank. Each rank has [ep_size] counters
    uint32_t* completion_flags[kMaxRanks];       // If completion_flags[target_rank][source_rank] ==
                                                 // *flag_val, then source
                                                 // rank has signaled the target rank
    void* recv_buffers[kMaxRanks][kMaxPayloads]; // Per-rank receive buffers for
                                                 // each payload
...
// Prepare for dispatch: zero send_counters, local_token_counter and increment
// flag_val
...
struct MoeA2ACombineParams
{
    ...
    // Prepare-only field: original payload tensor pointer used to stage into
    // workspace
    ...
    int* topk_send_indices;   // Top-K compact routing info per local token (size:
                              // [local_num_tokens, top_k]), dst index
                              // per k, -1 for duplicates
    int const* recv_counters; // [ep_size] number of valid tokens per source rank
                              // for this target
    ...
    uint32_t* completion_flags[kMaxRanks]; // If completion_flags[target_rank][source_rank] ==
                                           // *flag_val, then source
                                           // rank has signaled the target rank
    void const* recv_buffers[kMaxRanks];   // Per-rank receive buffers (only for
                                           // single payload)
