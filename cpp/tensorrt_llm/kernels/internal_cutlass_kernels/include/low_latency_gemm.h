...
inline std::ostream& operator<<(std::ostream& out, LowLatencyCutlassGemmConfig const& config)
{
  ...
  if (config.cutlass_gemm_config.is_tma_warp_specialized) {
    out << "tile_config_sm90_enum: "
        << int(config.cutlass_gemm_config.tile_config_sm90)
        << ", mainloop_schedule_enum: "
        << int(config.cutlass_gemm_config.mainloop_schedule)
        << ", epilogue_schedule_enum: "
        << int(config.cutlass_gemm_config.epilogue_schedule)
        << ", cluster_shape_enum: "
        << int(config.cutlass_gemm_config.cluster_shape);
  } else {
    out << "tile_config_enum: "
        << int(config.cutlass_gemm_config.tile_config_sm80)
        << ", split_k_style_enum: "
        << int(config.cutlass_gemm_config.split_k_style)
        << ", split_k_factor: " << config.cutlass_gemm_config.split_k_factor
        << ", stages: " << config.cutlass_gemm_config.stages;
  }
  out << config.cutlass_gemm_config
      << " kernel_schedule_enum: " << static_cast<int>(config.kernel_schedule);
  return out;
}
...
public:
  ...
  virtual void gemm(__nv_fp8_e4m3 *A, __nv_fp8_e4m3 *B, float alpha, float beta,
                    void const *C, void *D, int m, int n, int k,
                    float pdl_overlap_ratio, float prefetch_ratio,
                    ConfigType gemmConfig, char *workspacePtr,
...
template <typename T>
class CutlassLowLatencyFp8GemmRunner
    : public virtual CutlassLowLatencyFp8GemmRunnerInterface {
public:
  CutlassLowLatencyFp8GemmRunner();
  ~CutlassLowLatencyFp8GemmRunner() = default;
  // gemm A rowMajor,  B colMajor, C and D rowMajor
  void gemm(__nv_fp8_e4m3 *A, __nv_fp8_e4m3 *B, float alpha, float beta,
            void const *C, void *D, int m, int n, int k,
            float pdl_overlap_ratio, float prefetech_ratio,
            ConfigType gemmConfig, char *workspacePtr,
...
private:
  size_t dispatchToArch(__nv_fp8_e4m3 const *A, __nv_fp8_e4m3 const *B,
                        float alpha, float beta, void const *C, void *D, int m,
                        int n, int k, float pdl_overlap_ratio,
                        float prefetech_ratio, ConfigType gemmConfig,
