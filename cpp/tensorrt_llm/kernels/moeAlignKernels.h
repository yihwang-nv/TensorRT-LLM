...
/**
 * @brief Aligns token distribution across experts to be compatible with block
 *size for matrix multiplication.
 *
 * This kernel sorts tokens by expert assignment and pads the distribution to
 *match block size requirements.
 ...
 * @param topk_ids_dtype_size Size of the dtype (e.g., sizeof(int32_t) or
 *sizeof(int64_t))
 * @param sorted_token_ids Output tensor for sorted token indices
 * @param expert_ids Output tensor for expert IDs per block
 * @param num_tokens_post_pad Output tensor for total tokens after padding
 *(single int32)
