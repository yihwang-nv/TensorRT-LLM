...
//
// This function is used to launch a kernel to wait for the signal from cpu to
// gpu.
// The signal should be set by moeSetSignalForGpuStageHost before calling this
// function.
// After the signal is set, functions like moeStatisticDevice can be called.
//
// @param signal: the signal
// @param enabled: output flag on device memory to indicate if the statistic is
// enabled
...
// @brief host version of moeWaitSignalForGpuStageDevice, should only be used
// for tests.
//
// @param signal: the signal
// @param enabled: output flag on host memory to indicate if the statistic is
// enabled
...
// @brief host version of moeSetSignalForCpuStageDevice, should only be used for
// tests.
//
// @param signal: the signal
...
// @param isFirstStage: whether the current stage is the first stage (only first
// stage need shift window)
// @param isLastStage: whether the current stage is the last stage (only last
// stage need update load factor)
// @param gatheredRawExpertIds: the gathered raw expert ids, should have shape
// [numTotalTokens, metaInfo.topK]
...
//
// This function is used to launch a kernel to do the statistic for local
// tokens.
//
// @param metaInfo: the meta info
// @param numTotalTokens: the total number of tokens in localRawExpertIds
// @param localExpertTokenCount: the token count that each expert has for local
// tokens.
// @param enabled: flag on device memory to indicate if the statistic is enabled
// @param isFirstStage: whether the current stage is the first stage (only first
// stage need shift window)
// @param isLastStage: whether the current stage is the last stage (only last
// stage need update load factor)
// @param localRawExpertIds: the gathered raw expert ids, should have shape
// [numTotalTokens, metaInfo.topK]
...
//
// This function is used to launch a kernel to update the statistic info per
// iteration.
//
// @param metaInfo: the meta info
// @param statisticInfo: the statistic info
// @param globalExpertTokenCount: the global expert token count, should have
// shape [metaInfo.expertCount]
// @param enabled: flag on device memory to indicate if the statistic is enabled
...
//
// This function is used to launch a kernel to compute the route based on the
// token selected experts and the placement
// info.
//  For all input expert < 0 or >= metaInfo.expertCount, the route is set to
// invalid rank (metaInfo.epSize).
//
// @param metaInfo: the meta info
// @param placementInfo: the placement info
// @param tokenSelectedExperts: the selected experts of all tokenCount tokens,
// has shape of [tokenCount * topK]
// @param tokenRoutedRankIds: output the routed slotIds of all tokenCount
// tokens, has shape of [tokenCount * topK]
...
// The signal should be set by moeSetSignalForCpuStageDevice before calling this
// function.
