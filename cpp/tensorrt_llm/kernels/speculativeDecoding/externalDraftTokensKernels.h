...
//! \brief Accepts or rejects draft tokens based on their probability
// distributions or the equality of draft and target
//! tokens. Corrects targetLogits for the last accepted token
//! according to https://openreview.net/pdf?id=C9NEblP8vS
//!
//! \param batchSize current batch size
//! \param draftProbs output buffer [maxDraftTokens, batchSize, beamWidth,
// vocabSize].
//! Workspace buffer for token probabilities of the draft model.
//! \param targetProbs output buffer [maxDraftTokens+1, batchSize, beamWidth,
// vocabSize].
//! Workspace buffer for token probabilities of the target model.
//! \param numsDraftTokens input buffer [batchSize]. Number of draft tokens per
// request
//! \param batchUseDraftLogits input buffer [batchSize]. Acceptance logic using
// draft logits or not, per request
//! \param draftIds input buffer [batchSize, draftTokens]. Pointer to draft
// token ids.
//! \param finishedInput input buffer [batchSize, beamWidth].
//! \param finishedOutput output buffer [batchSize, beamWidth]. At each step
// sets SKIP_DECODING if token is not
//! accepted.
//! \param curandState input buffer [batchSize]. Curand states properly
// initialized using invokeCurandInitialize
//! per request.
//! \param batchSlots input buffer [batchSize], address map from local index to
...
//! \param randomThreshold True if use uniformly sampled threshold for token
// acceptance
//! \param constantThreshold threshold used to accept tokens if randomThreshold
// is false
//! \param step The current step of decoding (draft token id index)
//! \param batchIsAccepted output buffer [batchSize]. Stores acceptance result
// for multinomial sampling later or
//! forwarding next step.
//! \param targetOutputIds input/output buffer [batchSize]. Stores target
// sampling output ids for acceptById
//! logics.
//! \param stream stream
template <typename T>
...
//!
//! \param batchSize current batch size
//! \param targetLogits input/output buffer [batchSize][draftTokens+1,
// beamWidth, vocabSize].
...
//! \param outputIdsAfterSampling input buffer [batchSize, vocabSize]. Stores
// all selected IDs from sampling for
//! masking.
//! \param numsDraftTokens input buffer [batchSize]. Number of draft tokens per
// request
//! \param runtimeTopKDevicePtr input buffer [batchSize] the topks in sampling
// step, for porting topK ids out.
//! \param maskBuffer input buffer [batchSize, vocabSize] for masking
// calculation (index value to position).
//! \param stream stream
template <typename T>
