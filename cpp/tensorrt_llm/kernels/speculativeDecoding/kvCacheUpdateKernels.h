 ...
 * @param pastKeyValueList : Past key value list, which is the pointer array of
 * each KVLinear cache.
 ...
 * @param seqSlotRemapping mapping from batch index to index of the seqSlot in
 * the sorted seqSlot buffer
 * e.g. for requests [0, 1, 2] with seqSlots [5, 3, 4], seqSlotRemapping is [1,
 * 2, 0]
 * Required to match seqAcceptedDraftTokenOffsets and
 * packedAcceptedDraftTokensIndices from gptDecoderBatched
 ...
 * @param numKVHeads : Number of KV heads
 * @param sizeInBytesPerKVHead : Size of each KV head
 * @param rewindDraftTokenCounts : Pointer to an array of length seqCount, each
 * element indicated the rewind count of
 * one sequence.
 * @param seqSlotRemapping mapping from batch index to index of the seqSlot in
 * the sorted seqSlot buffer
 * e.g. for requests [0, 1, 2] with seqSlots [5, 3, 4], seqSlotRemapping is [1,
...
/*!
 * Update Linear KV cache using both common rewind and separate rewind count for
 * each sequence. The common
 * rewindDraftTokenCommonCount and rewind count of each sequence in
 * rewindDraftTokenSeparateAdjustments will be added
 * together for the final rewind count. It can save one add if both of them need
 * to be used.
 * @param seqAcceptedDraftTokenOffsets : Array of length seqCount + 1, like [0,
 * 3, 5]
 * @param packedAcceptedDraftTokensIndices : Array of length
 ...
 * @param rewindDraftTokenSeparateAdjustments : Pointer to an array of length
 * seqCount, each element indicated the
...
/*!
 * Update Block KV cache using both common rewind and separate rewind count for
 * each sequence. The common
 * rewindDraftTokenCommonCount and rewind count of each sequence in
 * rewindDraftTokenSeparateAdjustments will be added
 * together for the final rewind count. It can save one add if both of them need
