...
//! \brief Sets finished state to FinishedState::FINISHED_STOP_WORDS if any of
// the stopWords is met.
//!
//! \param outputIds input buffer [maxBatchSize][maxSeqLen]. Contains pointers
// to rows with output tokens per request
//! \param parentIds input buffer [maxBatchSize][maxSeqLen]. Contains pointers
// to rows with parent ids. Applicable when
//! beamWidth > 1
//! \param stopWords input buffer [maxBatchSize][2, stopWordsLen]. For each
// instance in batch the first row
//! is the token ids of the stop words. The second row is the exclusive prefix
...
//! When numNewTokens is not nullptr, it is updated to the first entrance of the
// found stop words.
//! \param batchSlots input buffer[batchSize], optional. Indices of rows of data
// in memory pool
//! \param stopWordsLen input buffer [maxBatchSize], cumulative length of all
// stop words per request
//! \param numNewTokens input/output buffer [maxBatchSize], optional, number of
// tokens predicted per step.
...
//! \brief Sets finished states based on the sequenceLimitLength and computes
// number of finished sequences in the batch.
//!
//! \param finished input/output buffer [maxBatchSize, beamWidth]. Finished
// states. Set to
...
//! \param sequenceLimitLength input buffer [maxBatchSize]. Maximum sequence
// length.
//! \param sequenceLengths input/output buffer [maxBatchSize, beamWidth].
//! Current sequence lengths of the request tokens.
//! \param numNewTokens output buffer [maxBatchSize], optional. Number of tokens
// per step for each request.
...
//! \brief Sets finished states based on the endIds and adjusts sequence length
// to length before the first EOS token.
...
//! [maxBatchSize, beamWidth]. Finished states. Set to
// FinishedState::FINISHED_EOS if any new tokens contain EOS.
//! \param sequenceLengths input/output buffer [maxBatchSize, beamWidth].
//! Current sequence lengths of the request tokens.
//! \param numNewTokens input/output buffer [maxBatchSize], optional. Number of
// tokens per step for each request.
//! It is assumed that all requests have maxTokensPerStep tokens per step if
// nullptr.
//! \param batchSlots input buffer[batchSize], optional. Indices of rows of data
// in memory pool
