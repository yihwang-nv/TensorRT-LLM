...
//! `setup` repeatedly, but once per request; `forward*` repeatedly, many
// times per request.
//! A possible sequence would be, construct(maxBatchSize) -> setup({1,3}) ->
// forward({1, 3})
//! -> forward({1, 3}) -> setup({2,4}) -> forward({1, 3, 2, 4}) -> forward({1,
// 3, 2, 4})
//! -> forward({1, 2, 4}), where {a,b} are batchSlots, and {3} ends at last
// step.
//! As a result there are three types of batches.
//! 1. `maxBatchSize` for each layers to reserve resources.
//!    It is passed through class constructor, in
// DecoderDomain.getBatchSize().
//! 2. `setupBatchSize` for setting up layers for a batch of new requests.
//!    It is passed through `setup` method.
//! 3. `forwardBatchSize` for layers forwarding for a batch of existing active
// requests.
...
struct LookaheadSetupParams : public DecodingSetupParams
{
    ...
    std::vector<executor::LookaheadDecodingConfig> algoConfigs; // [1] or
                                                                // [batchSize]
...
public:
    ...
    std::optional<TensorConstPtr> badWordsPtr;     // [maxBatchSize][2,
                                                   // bad_words_length], on gpu
...
public:
    ...
    std::optional<TensorConstPtr> stopWordsPtr;        // [maxBatchSize][2,
    // stop_words_length], on pinned
...
public:
    ...
    //! Need both of these fields to support legacy code during transition
    // period to the batched decoder
    std::optional<TensorConstPtr> logits; // [forwardBatchSize, beamWidth,
                                          // vocabSizePadded], on gpu
    OptVec<TensorConstPtr> logitsVec;     // [forwardBatchSize][beamWidth,
                                          // vocabSizePadded], on gpu
    TensorConstPtr batchSlots;            // [forwardBatchSize], on pinned

    // optional parameters
    std::optional<TensorPtr> srcCacheIndirection; // [forwardBatchSize,
                                                  // maxBeamWidth, maxSeqLen],
                                                  // on gpu, for Beam Search
    std::optional<TensorConstPtr> embeddingBias;  // [vocabSizePadded], on gpu
    std::optional<TensorConstPtr> inputLengths;   // [maxBatchSize, maxBeamWidth],
                                                  // on gpu
    ...
    OptVec<runtime::SizeType32> beamSearchSteps; // [forwardBatchSize], for
...
public:
    ...
    // [maxBatchSize][maxDraftPathLen][maxDecodingTokens, vocabSizePadded], on
    // gpu
...
public:
    ...
    //! Draft tokens for the next iteration. The first token in each path is the
    // last accepted token at current
    //! iteration. E.g. if forwardBatchSize == 1, maxNumPaths == 2, maxPathLen==
    // 3, [[[0, 1, 2], [0, 1, 10]]]
    TensorConstPtr nextDraftTokens; // [forwardBatchSize, maxNumPaths,
                                    // maxPathLen], gpu
    //! Compressed form of `nextDraftTokens`, where common prefixes and
    // collapsed.
    //! Using example above [0, 1, 2, 10]
    TensorConstPtr nextFlatTokens; // [forwardBatchSize * maxDecodingTokens],
                                   // gpu
    //! Indices of draft tokens in the compressed `nextFlatTokens` for the next
    // iteration.
    //! Using example above, [[[0, 1, 2], [0, 1, 3]]]
    TensorConstPtr nextDraftIndices; // [forwardBatchSize, maxNumPaths,
                                     // maxPathLen], gpu
    //! Probabilities of the next draft tokens.
    TensorConstPtr nextDraftProbs; // [forwardBatchSize, maxNumPaths,
                                   // maxDraftPathLen, vocabSize], gpu
    //! Same as `nextDraftTokens`, but for current iteration.
    //! Current accepted tokens obtained as
    //`lastDraftTokens[bi][bestPathIndices[bi]][1:bestPathLengths[bi]]`.
    TensorConstPtr lastDraftTokens; // [forwardBatchSize, maxNumPaths,
                                    // maxPathLen], gpu
    //! Same as `nextDraftIndices`, but for current iteration.
    TensorConstPtr lastDraftIndices; // [forwardBatchSize, maxNumPaths,
                                     // maxPathLen], gpu
    //! Boolean attention masks.
    //! maxDecodingTokens' = generationLengths.max()
    TensorConstPtr masks; // [forwardBatchSize, maxDecodingTokens',
                          // maxDecodingTokens'], gpu
    //! Relative to `positionIdsBase` position ids. Same as `nextFlatTokens` for
    // next draft indices.
    //! Using example above, [0, 1, 2, 3]
    TensorConstPtr packedPosIds; // [forwardBatchSize * maxDecodingTokens], gpu
    //! Lengths of the accepted paths for each request. It is 1 for context
    // phase (Only 1 primary tokens is accepted).
    TensorConstPtr bestPathLengths; // [forwardBatchSize], gpu
    //! Indices of the accepted paths for each request. It is 0 for context
    // phase.
    ...
    //! It is not the same as batchSlots because it maps the ordered engine
    // outputs to the respective seqSlot,
...
public:
    ...
    TensorConstPtr nextDraftTokens; // [forwardBatchSize,
                                    // maxDecodingDraftTokens], gpu
    //! Number of the draft tokens for the next iteration.
    TensorConstPtr nextDraftLens; // [forwardBatchSize], gpu
    //! Draft paths for the next iteration.
    TensorConstPtr nextDraftPaths; // [forwardBatchSize, maxDecodingTokens,
                                   // maxPathLen], gpu
    //! Same as `nextDraftTokens`, but for current iteration.
    TensorConstPtr lastDraftTokens; // [forwardBatchSize, maxNumPaths,
                                    // maxPathLen], gpu
    ...
    TensorConstPtr lastDraftPaths; // [forwardBatchSize, maxDecodingTokens,
                                   // maxPathLen], gpu
...
public:
    ...
    //! Optional parameters
    std::optional<TensorPtr> finished;       // [maxBatchSize * maxBeamWidth], on
                                             // pinned
    std::optional<TensorPtr> sequenceLength; // [maxBatchSize * maxBeamWidth],
                                             // on gpu
    std::optional<TensorPtr> cumLogProbs;    // [maxBatchSize * maxBeamWidth], on
                                             // gpu, for Beam Search
    std::optional<TensorPtr> outputLogProbs; // [maxBatchSize, maxBeamWidth,
                                             // maxSeqLen], on gpu
    std::optional<TensorPtr> parentIds;      // [maxBatchSize, maxBeamWidth,
    // maxSeqLen], on gpu, for Beam Search

    TensorPtr outputIdsPtr;     // [maxBatchSize][maxBeamWidth, maxSeqLen], on gpu
                                // and outputIdsPtr[i], on gpu
    TensorPtr outputIdsPtrHost; // [maxBatchSize][maxBeamWidth, maxSeqLen], on
                                // cpu but outputIdsPtrHost[i], on gpu
    TensorPtr parentIdsPtr;     // [maxBatchSize][maxBeamWidth, maxSeqLen], on cpu
                                // but parentIdsPtr[i], on gpu
    TensorPtr newTokens;        // [maxBatchSize, maxBeamWidth], on gpu, tokens
                                // predicted at current iteration.

    // optional parameters
    std::optional<TensorPtr> numNewTokens;        // [maxBatchSize], on pinned, number
                                                  // of tokens predicted at current
                                                  // iteration
    std::optional<TensorPtr> finishedSum;         // [1], on pinned
    std::optional<TensorPtr> outputLogProbsTiled; // [maxSeqLen, maxBatchSize,
                                                  // maxBeamWidth], on gpu
...
public:
    ...
    TensorPtr tgtCacheIndirection; //[forwardBatchSize, maxBeamWidth,
    // maxSeqLen], on gpu, the k/v cache index

    std::unique_ptr<kernels::BeamHypotheses> beamHypotheses; // Structure
                                                             // maintains
                                                             // variables of
                                                             // Beam Search
};
...
//!
//! For one example sequence [a, b] [c] <x, y, z>, where, [a, b, c] is the
// accepted sequence,
//! [c] is the last accepted token, and <x, y, z> is the draft tokens from
//`nextDraftTokens` saved by last step.
//! [c]'s position id is known, only position ids for <x, y, z> need to be
// provided in `nextDraftPosIds`.
//! LLM inputs {c, x, y, z} and generates {c', x', y', z'}.
//!
//! {c'} is always accepted and {x', z'} is supposed to be accepted.
//! The accepted tokens [c', x', z'] is saved in `outputIds` in-place,
// starting from `sequenceLength`.
...
//!
//! For a batchSlots {1, 3}, `numNewTokensCumSum` is an exclusive sum of
//`numNewTokens` over the batch,
//! the `numNewTokens` may be {3, 5}, `numNewTokensCumSum` is {0, 3, 8}.
//!
//! `nextDraftLengths` and `prevDraftLengths` are needed for methods that
// support if variable
//! draft length. `nextDraftLengths` must contain the number of draft tokens
// per request for the next iteration.
//! `prevDraftLengths` must contain the number of draft tokens used in the
// current iteraiton.
//!
//! `pathsOffsets` is needed for KV cache rewind. It contains the positions of
// the accepted draft tokens in the
//! flattened tensor of draft tokens. E.g. if for sequence {c, x, y, z} only
//`y` and `z` were accepted,
//! `pathsOffsets` contains [1, 2]. `pathsOffsets` is flattened tensor for
// whole batch.
//!
//! The order of `pathsOffsets` and `numNewTokensCumSum` must be aligned. Such
// that
//! `pathsOffset[numNewTokensCumSum[bi]:numNewTokensCumSum[bi+1]]` is the
// slice of offsets for `bi`th request.
//! Furthermore, the order of requests is important and must be aligned with
// sorted `RuntimeBuffers::seqSlots`
//! such that the request with smaller `seqSlot` stays earlier in the tensors.
//! However, this condition usually holds if method does not expect from the
// engine anything else, but logits.
...
public:
    ...
    //! Prev step draft tokens lengths, should be filled only for variable draft
    // length speculative decoding mode
    TensorPtr prevDraftLengths; // [maxBatchSize]
    //! Next step draft tokens lengths, should be filled only for variable draft
    // length speculative decoding mode
    ...
    TensorPtr packedMasks;        // [maxBatchSize, maxDecodingTokens,
                                  // divUp(maxDecodingTokens, 32)]
};
...
public:
    ...
    //! for TLLM engine input "spec_decoding_generation_lengths", indicating how
    // many tokens to be generated.
    //! currently, the 1st step of generation is 1, set at `setup`, others are
    // maxDecodingTokens, set at `forward`.
    ...
    //! ABC<D>efgxyz--- // sequence tokens, ABCD: golden; efg, xyz: draft; ---:
    // padding.
...
public:
    ...
    //! Draft tokens for the next iteration. The first token in each path is the
    // last accepted token at current
    //! iteration. E.g. if batchSize == 1, maxNumPaths == 2, maxPathLen== 3,
    //[[[0, 1, 2], [0, 1, 10]]]
    TensorPtr unpackedNextDraftTokens; // [maxBatchSize, maxNumPaths,
                                       // maxPathLen], on gpu
    //! Indices of draft tokens in the compressed `nextFlatTokens` for the next
    // iteration.
    //! Using example above, [[[0, 1, 2], [0, 1, 3]]]
    TensorPtr unpackedNextDraftIndices; // [maxBatchSize, maxNumPaths,
                                        // maxPathLen], on gpu
    //! Probabilities of the next draft tokens.
    TensorPtr nextDraftProbs; // [maxBatchSize, maxNumPaths, maxPathDraftLen,
                              // vocabSize], on gpu
    //! Baseline for the position ids.
    ...
    TensorPtr randomDataSample; // [maxBatchSize], on gpu
    //! Randomly sampled data (between 0.f and 1.f)
    TensorPtr randomDataValidation; // [maxBatchSize, maxNumPaths,
                                    // maxDraftPathLen], on gpu
...
public:
    ...
    TensorPtr unpackedNextDraftTokens; // [maxBatchSize,
                                       // maxDecodingDraftTokens], on gpu
    //! Draft paths for the next iteration.
