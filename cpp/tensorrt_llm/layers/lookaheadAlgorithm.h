...
public:
    ...
    //! @brief Currently the resource management is to be aligned with batch
    // manager.
    //! @param w, n, g is the Jacobi window, n-gram level and guess set size
    // respectively.
    LookaheadAlgorithm(
    ...
    //! LookaheadDecodingLayer need call once for the first token in generation
    // phase.
    void accept(TensorConstPtr const& generatedTokens);

    //! @brief combine lookahead and guess to prepare the tensors.
    //! input @param lastPositionIdPtr is position id of the last golden token,
    // in a TensorPtr.
    //! input @param lastTokenPtr the last golden token for searching in the
    // pool, in a TensorPtr.
    //! output @param draftTokens, positionIds includes the lookahead and the
    // verification branch information.
    //! output @param draftLengthPtr holds the draft tokens length.
    //! output @param attentionMask holds the draft tokens dependency mask, and
    // attentionMaskOffset is the index offset
    //! in attentionMask.
    ...
    //! @brief update the internal states and generate accepted tokens from
    //@param outputTokens.
    //! input @param sampledTokens is the all the tokens from the language
    // model.
    ...
    //! inplace encode the @param tokens and @param posIds according to
    // attention @param masks, and record the offsets
    //! in @param encodeMap.
...
private:
    ...
    //! @brief generate verification branch information. Also save the guessed
    // tokens for future verification.
    ...
    //! @brief verify the guessed tokens results and generate the longest
    // accepted tokens.
    //! input @param newLastToken is the new-generated last golden token.
    //! input @param sampledTokens is the generated token results from the
    // language model.
    //! input @param endToken is the end token for early quit detection.
    //! output @param accepted in @param acceptedLength, including the first
    // golden one.
    //! output @param acceptedOffsets is the offsets of draft tokens, excluding
