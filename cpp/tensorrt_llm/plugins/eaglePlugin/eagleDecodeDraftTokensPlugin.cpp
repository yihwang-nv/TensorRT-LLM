    ...
    int outputIndex, nvinfer1::DimsExprs const* inputs, int nbInputs, nvinfer1::IExprBuilder& exprBuilder) noexcept
{
    ...
    auto const maxDecodingDraftTokensSquareExpr
        = exprBuilder.operation(DimensionOperation::kPROD, *maxDecodingDraftTokensExpr,
            *maxDecodingDraftTokensExpr); // maxDecodingDraftTokensExpr *
    ...
    else
    {
        TLLM_CHECK_WITH_INFO(false,
            "Wrong outputIndex %d in "
            "EagleDecodeDraftTokensPlugin::"
            "getOutputDimensions",
            outputIndex);
    }
    return ret;
    ...
    int pos, nvinfer1::PluginTensorDesc const* inOut, int nbInputs, int nbOutputs) noexcept
{
    ...
    {
        // input: path, num_valid_logits, use_dynamic_tree, dynamic_tree_max_topK,
        // input_draft_token_ids,
        //        input_draft_lens, input_current_expand_index,
        // input_all_layers_draft_token_ids
        // output: output_draft_token_ids, output_draft_lens, output_path,
        // output_next_expand_index
        //        output_all_layers_draft_token_ids,
    ...
    nvinfer1::PluginTensorDesc const* outputs, int nbOutputs) const noexcept
{
    ...
    if (mTopKSampling)
    {
        ...
        auto const draftTokenSamplingWorkspaceSize = getTopKWorkspaceSize<T>(numInputLogits, /* maxTokensPerStep */ 1,
            /* maxTopK */ maxTopK, vocabSizePadded);
        ...
        auto const secondTopKSamplingWorkspaceSize = getTopKWorkspaceSize<float>(batchSize, /* maxTokensPerStep */ 1,
            /* maxTopK */ maxTopK, maxTopK * maxTopK);

        // 10. Eagle-2, the outputIds of the second topK sampling, shape [batchSize,
        // maxDecodingTokens]
        auto const secondTopKOutputIdsSize = batchSize * maxDecodingTokens * sizeof(TokenIdType);
        // 11. Eagle-2, the outputIdsPtr of the second topK sampling, shape
        // [batchSize]
        auto const secondTopKOutputIdsPtrSize = batchSize * sizeof(TokenIdType*);
        // 12. Eagle-2, the inputScoresPtrs of the second topK sampling, shape
        // [batchSize]
        auto const secondTopKInputScoresPtrsSize = batchSize * sizeof(float*);
        // 13. Eagle-2, the outpuLogProbs of the second topK samplig, shape
        // [batchSize, maxDecodingDraftTokens]
        auto const secondTopKOutputLogProbsSize = batchSize * maxDecodingDraftTokens * sizeof(float);

        // 14. Eagle-2, the input scores pointers of the third topK sampling, shape
        // [batchSize]
        // Each points to a vocabSize = '(mNumEagleLayers - 1) * dynamicTreeMaxTopK
        // * dynamicTreeMaxTopK +
        // dynamicTreeMaxTopK'
        auto const thirdTopKInputScoresPtrsSize = batchSize * sizeof(float*);
        // 15. Eagle-2, the output of the third topK sampling, shape [batchSize,
        // maxDecodingDraftTokens]
        auto const thirdTopKOutputIdsSize = batchSize * maxDecodingDraftTokens * sizeof(TokenIdType);
        // 16. Eagle-2, the output pointers of the third topK sampling, shape
        // [batchSize]
        auto const thirdTopKOutputIdsPtrsSize = batchSize * sizeof(TokenIdType*);
        // 17. Eagle-2, the workspace of the third topK sampling
        // Sampling from [batchSize, '(mNumEagleLayers - 1) * dynamicTreeMaxTopK *
        // dynamicTreeMaxTopK +
        // dynamicTreeMaxTopK'] to [batchSize, maxDecodingDraftTokens] We over-set
        // the vocabsize here.
        ...
        // The real topK value is min(maxDecodingDraftTokens,
        // totalNumDraftTokensForAllLayers)
    ...
    cudaStream_t stream) noexcept
{
    ...
    // We allocate many buffers with 'numInputLogits' size, but the input logits
    // will include some padding logits.
    ...
    // In Eagle-2, dynamicTreeMaxTopK is equal to maxNonLeavesPerLayer in the
    // internal EagleNets.
    ...
    // The indices of the nodes that will be expand in this layer, shape:
    // [batchSize, maxDecodingDraftTokens]
    // The index is related to the final output tree, which has
    // max_decoding_draft_tokens draft tokens.
    ...
    // shape: [batchSize, mNumEagleLayers, maxDecodingDraftTokens x
    // maxDecodingDraftTokens]
    ...
    // Update the predecessor of the draft tokens, shape: [batchSize,
    // mNumEagleLayers, maxDecodingDraftTokens x
    // maxDecodingDraftTokens]
    ...
    auto const samplingWorkspaceSize = getTopKWorkspaceSize<T>(numInputLogits, /* maxTokensPerStep */ 1,
        /* maxTopK */ maxTopK, vocabSizePadded);
    ...
    // Workspace 2: topKOffset tensor: shape: [batchSize], number of nodes that
    // have successors for each requests
    ...
    // Workspace 4: outputIds pointers tensor: shape [numInputLogits], each points
    // to a [maxDecodingDraftTokens] buffer
    ...
    // Workspace 5: outputIds tensor: flatten outputIds, shape [numInputLogits *
    // maxDecodingDraftTokens]
    ...
    // Workspace 6: number of successors for each nodes tensor: shape [batchSize *
    // maxDecodingTokens]
    ...
    if (useDynamicTree)
    {
        ...
        // Which will be updated with the previous layer's scores (i.e.,
        // pluginInputPrevScores), and will be treat as
        // the input of the second topK sampling. For mLayerIdx == 0, shape:
        // [numInputLogits(batchSize),
        // maxDecodingDraftTokens] For mLayerIdx > 0, shape:
        // [numInputLogits(batchSize * dynamicTreeMaxTopK),
        // maxDecodingDraftTokens]
    ...
    // Workspace 9: Sampling from [batchSize, dynamicTreeMaxTopK *
    // maxDecodingDraftTokens] to [batchSize,
    // dynamicTreeMaxTopK]
    auto const secondTopKSamplingWorkspaceSize = getTopKWorkspaceSize<float>(batchSize, /* maxTokensPerStep */ 1,
        /* maxTopK */ maxTopK, secondTopKVocabSize);
    ...
    // Workspace 10: the second (scores) sampling's outputIds, shape: [batchSize,
    // maxDecodingDraftTokens]
    ...
    // Workspace 14: The input scores pointers of the third topK sampling, shape
    // [batchSize]
    float** thirdTopKInputScoresPtrs
        = reinterpret_cast<float**>(tc::nextWorkspacePtr(workspaceBytePtr, offset, batchSize * sizeof(float*)));

    // Workspace 15: The output of the third topK sampling, shape [batchSize,
    // maxDecodingDraftTokens]
    TokenIdType* thirdTopKOutputIds = reinterpret_cast<TokenIdType*>(
        tc::nextWorkspacePtr(workspaceBytePtr, offset, batchSize * maxDecodingDraftTokens * sizeof(TokenIdType)));

    // Workspace 16: The output pointers of the third topK sampling, shape
    // [batchSize]
    ...
    // Workspace 18. Eagle-2, the topKs for each request in the third topK
    // sampling, shape [batchSize]
    // The real topK value is min(maxDecodingDraftTokens,
    // totalNumDraftTokensForAllLayers)
    ...
    if (useDynamicTree)
    {
        // For Eagle-2, the topK value between different requests are the same, all
        // set to 'dynamicTreeMaxTopK'.
        ...
        // We set the 'batchSize' and 'maxBatchSize' to 'numInputLogits', while
        // 'numInputLogits' logits may contain
        // some padding logits, which do not need to be calculated.
        // We use 'skipDecode' list to skip these padding logits. This could avoid
    ...
    if (useDynamicTree)
    {
        ...
        // We take the outputLogProbs of the first topK sampling as the scores
        // directly.
        if (mLayerIdx != 0)
        {
            // Update firstTopKOutputLogProbs with pluginInputPrevScores, which is the
            // scores from the previous layer
            ...
            // Do the second top-dynamicTreeMaxTopK sampling among this
            // dynamicTreeMaxTopK x dynamicTreeMaxTopK draft
            // tokens. Through the second topK sampling, we obtain the
            // dynamicTreeMaxTopK output draft tokens of this
            // layer.

            // Although theoretically we only need to select 'dynamicTreeMaxTopK'
            // draft tokens from 'dynamicTreeMaxTopK
            // * dynamicTreeMaxTopK' draft tokens, we over-set vocabSize here. This is
            // because when we write the scores
            // into firstTopKOutputLogProbs, we store it in the form of [batchSize *
            // dynamicTreeMaxTopK,
            // maxDecodingDraftTokens]. For each request, these 'dynamicTreeMaxTopK *
            // dynamicTreeMaxTopK' scores are not
            // saved continuously, but in the format of [dynamicTreeMaxTopK,
            // maxDecodingDraftTokens]. For unused
            // positions, we set '-inf' to ensure that they will not be sampled.
            // Examples: For a request,
            // dynamicTreeMaxTopK == 3, the scores in its buffer ([dynamicTreeMaxTopK,
            // maxDecodingDraftTokens]) are as
            // follow:
            // [[1.1, 2.2, 3.3, -inf, -inf, ...],
            //  [4.4, 5.5, 6.6, -inf, -inf, ...],
            //  [7.7, 8.8, 9.9, -inf, -inf, ...]]
        ...
        // 2) Copy dynamicTreeMaxTopK (or dynamicTreeMaxTopK * dynamicTreeMaxTopK)
        // draft tokens to
        // pluginOutputAllLayersDraftTokenIds 3) Set the predecessors of these draft
        // tokens and save to
        // pluginOutputAllLayersDraftTokenIdsPredecessor,
        //    which will be used to reconstruct the final output tree at the last
        // layer
        invokeCopyScoresAndDraftTokenIds(mLayerIdx, mNumEagleLayers, maxDecodingDraftTokens, batchSize,
            dynamicTreeMaxTopK,
            pluginInputCurrentExpandIndices, // The indices of the nodes that expand
                                             // in this layer (i.e., the input
        ...
        // Update Path
        // For mLayerIdx == 0, the output of the first topK sampling are the output
        // draft tokens of this layers. The
        // update logic is simple. For mLayerIdx > 0, the output of the second topK
        // sampling are the output draft tokens
        // of this layers. 'secondTopKOutputIdsPtrs' contains the
        // top-dynamicTreeMaxTopK selected from the second topK
        // sampling. 'pluginOutputNextExpandIndices' record the selected the
        // top-dynamicTreeMaxTopK draft token's Id of
        // this layer,
        //     which will be used in the next layer to compute the predecessors.
        // The last layer will completely reconstruct the paths, so there is no need
        // to update the paths here.
        if (mLayerIdx != mNumEagleLayers - 1)
        {
            invokeUpdatePath(mLayerIdx, batchSize, dynamicTreeMaxTopK, maxDecodingTokens, maxPathLen, pluginInputPaths,
                pluginOutputPaths,
                secondTopKOutputIdsPtrs, // if mLayerIdx == 0, secondTopKOutputIdsPtrs
                                         // == nullptr, and it's useless
                                         // during update paths
        ...
        if (mLayerIdx != 0)
        {
            // We will extract the real draft tokenIds and scores from
            // 'firstTopKOutputIdsFlatten' and
            // 'secondTopKInputScoresPtrs' according to the 'secondTopKOutputIdsPtrs'.
            // And store them into
            // 'secondTopKOutputIdsPtrs' and 'secondTopKOutputLogProbs' (reuse these
            // buffers).
            // secondTopKInputScoresPtrs: shape [batchSize * dynamicTreeMaxTopK,
            // maxDecodingDraftTokens]
            //     The original scores, which were used to do the second TopK sampling
            // secondTopKOutputIdsPtrs: shape [batchSize], each points to a
            // [maxDecodingDraftTokens] buffer
            //     The output of the second TopK sampling, which are the indices of
            // the top-dynamicTreeMaxTopK among
            //     'dynamicTreeMaxTopK * dynamicTreeMaxTopK'. We need to figure out
            // what these top-dynamicTreeMaxTopK
            //     draft tokens' real tokenIds.
            // firstTopKOutputIdsFlatten: shape [batchSize * dynamicTreeMaxTopK,
            // maxDecodingDraftTokens]
        ...
        // if mLayerIdx == 0, directly use the first topK's outputIds / logProbs as
        // this layer's output draft tokens /
        // scores if mLayerIdx > 0, we use the second topK's outputIds / logProbs,
        //      which is updated with the real draft tokenIds / logprobs in
        // 'invokeExtractScoresAndRealDraftTokensIds'
        ...
        if (mLayerIdx == mNumEagleLayers - 1)
        {
            ...
            // When reach the last EagleNet, we need to do the third sampling, which
            // take all layers' draft tokens and
            // scores as input, and then select top-maxDecodingDraftTokens draft
            // tokens among them. We need to
            ...
            params.maxTopK = maxDecodingDraftTokens; // We set maxTopK to
                                                     // 'maxDecodingDraftTokens' to
                                                     // align the
    ...
    {
        // Eagle-1: Copy output token id from outputIdsPtrs to the plugin output
        // buffer
