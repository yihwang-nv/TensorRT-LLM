...
// request).
// num_gen_requests = number of generation requests (beam_width sequences per
// request).
// Context sequences have to appear first, generation sequences after

// inputs (see GPTAttentionPlugin::isEntryUsed for when each tensor is
// actually used)
//     0.  input_tensor [batch_size, seq_len, local_hidden_size + 2 *
// local_num_kv_heads * head_size] or
//                      [num_tokens, local_hidden_size + 2 *
// local_num_kv_heads * head_size] when
...
//     6.  cache_indir [num_gen_requests, beam_width, memory_max_len]
// (required in beamsearch) (optional)
//     7.  host_request_types [batch_size] int32. 0: context; 1: generation:
// 2: none. When not in inflight-batching
//     mode,
//                      all elements must be identical.
//     8.  past_key_value_pool [batch_size, 2, local_num_kv_heads,
// max_seq_len, head_size] or
//         block_offsets [batch_size, 2, max_blocks_per_seq] if paged kv cache
// (optional)
//     8.1 host_block_offsets [batch_size, 2, max_blocks_per_seq] if paged kv
// cache (optional)
...
//     13. attention_packed_mask [num_tokens, kv_seqlen / 32] (on device,
// uint32_t, optional)
//          - pack masks by encoding multiple mask positions into a single
// 32-bit unsigned integer.
//          - see kernels/contextMultiHeadAttention/fmhaPackedMask.cpp for
// more details.
//     14. rotary_inv_freq [head_size / 2] or [head_size] (longrope type)
// (float) (on device, optional)
//     15. rotary_cos_sin [max_num_embedding_positions, 2] (float) (on device,
// optional)
//     16. alibi_slopes [num_heads] (optional for ALiBi position embedding)
//     17. relative_attention_bias [num_heads] (optional for ALiBi position
// embedding)
//     18. host_context_lengths [batch_size] int32. (optional, required when
// remove_input_padding is true)
//     19. qkv_bias (optional) [local_hidden_size * 3]
//     20. spec_decoding_generation_lengths (optional, required when medusa is
// enabled) (int32_t) [batch_size]
//     21. spec_decoding_packed_mask (optional, required when medusa is
// enabled) (int32_t) [num_tokens, packed_mask_dim]
//                                    packed_mask_dim =
// divUp(max_num_spec_decoding_tokens + 1, 32)
//     22. spec_decoding_position_offsets (optional, required when medusa is
// enabled) (int32_t) [batch_size,
//     max_num_spec_decoding_tokens + 1]
//     23. spec_decoding_use (optional, bool) [1]: If it is set as true,
// enable speculative decoding
//     24. long_rope_rotary_inv_freq [head / 2] (float) (on device, optional)
//     25. long_rope_rotary_cos_sin [max_num_embedding_positions, 2] (float)
// (on device, optional)
//     26. host_runtime_perf_knobs (int64)
//     27. host_context_progress (void*)
//     28. position_id_tensor(MLA) [total_tokens], used for rope embedding in
// MLA
//     29. q_a_proj_tensor(MLA) [hidden_dim, c_q_dim + c_k_dim + ropd_dim],
// used to proj compacted QKV
//     30. q_a_layernorm_tensor(MLA) [c_q_dim], rmsnorm weight for compacted q
//     31. q_b_proj_tensor(MLA) [c_q_dim, head_num * head_size], weight for
// companted q to q in context
//     32. kv_a_proj_with_mqa_tensor(MLA) [c_q_dim, head_num * (c_k_dim +
// rope_dim)], weight for companted q to kdim in
//     generation
//     33. kv_a_layernorm_tensor(MLA) [c_k_dim], rmsnorm weight for compacted
// kv
//     34. kv_b_proj_tensor(MLA) [c_k_dim, head_num * 2 * (head_size -
// rope_dim)], weight for compacted kv to kv in
//     context
//     35. skip_attn (optional, bool) [1]: If it is set as true, skip the
// atteniton plugin and return
//     directly.
//
// outputs
//     output_tensor [batch_size, seq_len, local_hidden_size]
//     present_key_value_pool (optional if not paged kv cache) [batch_size, 2,
// local_num_kv_heads, max_seq_len,
//     head_size]
...
private:
    ...
    // Get generation input sequence length (might be larger than 1 in the
