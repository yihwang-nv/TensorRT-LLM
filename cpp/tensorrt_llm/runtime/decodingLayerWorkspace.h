...
public:
    ...
    ///@brief Gets a pointer to the start of the shared device workspace, as a
    ...
    ///@brief Gets a tensor with the given shape and type at the start of the
    /// device workspace.
    TensorPtr getWorkspaceAsDeviceTensor(ITensor::Shape shape, nvinfer1::DataType type);

    /// @brief A convenience function to copy the content of a standard vector
    /// to a device workspace.
        ...
        runtime::IBuffer::SharedPtr workspace)
    {
        ...
        TLLM_CHECK_WITH_INFO(sizeOfSrcInBytes <= sizeOfWorkspaceInBytes,
            "The size of the workspace (%zu bytes) is "
            "insufficient for the data (%zu bytes)",
            sizeOfWorkspaceInBytes, sizeOfSrcInBytes);
        ...
        TLLM_CHECK_WITH_INFO(sizePerWorkspaceElement == 1 || sizePerWorkspaceElement == sizeof(T),
            "Copy to typed workspace, but element size "
            "mismatched (src: %zu, workspace: %zu)",
            sizeof(T), sizePerWorkspaceElement);
    ...
    ///@brief Ensures the workspace has at least the provided space in bytes.
    /// Does nothing if the workspace is already
    /// at least as large.
    void resize(size_t minSize);

    ///@brief Given a collection of tuples of tensor shapes and data types,
    /// returns the memory aligned size required to
    ...
    ///@brief Given a collection of tensors, creates tensors with the same shape
    /// and data types in the workspace and
    ...
    /// @brief A convenience function to initialize curand states from a
    /// provided seed.
...
private:
    ...
    TensorPtr mBatchSlotsDevice;      // <! A copy of the batch slots on device
                                      // ensure fast access when used in kernels.
    TensorPtr mRuntimeLogitsDevice;   // <! The working state of the logits while
                                      // decoding.
    TensorPtr mCurandStatesDevice;    // <! The state information of the random
                                      // number generators for sampling based
                                      // decoding.
    BufferPtr mWorkspaceDeviceBuffer; // <! A buffer to be used as scratch space
                                      // by the decoding layers.
